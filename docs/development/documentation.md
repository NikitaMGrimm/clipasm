...
## Generated built-in program reference

The non-published `clipasm-reference-docs` workspace tool renders the built-in
program index, one page per built-in, and the delimited program-navigation block
in `docs/SUMMARY.md` from `clipasm::reference`.

Update the checked-in files after an intentional catalog change:

```console
cargo run --locked -p clipasm-reference-docs -- write
```

Check catalog examples, generated ownership, navigation, obsolete pages, and
exact generated bytes without changing files:

```console
cargo run --locked -p clipasm-reference-docs -- check
```

Do not edit generated program pages or the delimited navigation block directly.
