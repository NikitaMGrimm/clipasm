...
Canonical source already models source files as typed stack programs, and the
frontend boundary permits future authoring representations. Reusable authored
programs therefore need to become ordinary callable program definitions rather
than a YAML-specific include or macro mechanism.
...
A frontend produces a representation-neutral `SourcePackage` containing one
root source unit and its linked imported units. Each source unit defines one
callable source program. Imports bind explicit local aliases to other source
units; aliases are not re-exported and may not shadow built-ins.
...
For the YAML frontend, import paths are resolved relative to the importing
file, parsed units are deduplicated by canonical filesystem path, and import
cycles are rejected during package loading. Imported files may not declare
...
- YAML-to-YAML calls use ordinary program binding and stack semantics.
- Future frontends can produce the same package, imports, signatures, and
  bodies without changing compiler behavior.
...
- Import cycles such as `yaml1 -> yaml2 -> yaml3 -> yaml1` fail before output
