...
def generated_program_paths(root: Path) -> set[str]:
    directory = root / "docs/reference/programs"
    paths: set[str] = set()
    for path in sorted(directory.glob("*.md")):
        if not path.read_text(encoding="utf-8").startswith(GENERATED_PROGRAM_MARKER):
            print(
    ...
    if not paths:
        print(
            "package contents check failed: no generated program pages found",
            file=sys.stderr,
        )
        raise SystemExit(1)
    return paths
...
def main() -> int:
    ...
    required = REQUIRED_PATHS | generated_program_paths(root)
    ...
    if forbidden:
        ...
        return 1
    print(
        "package contains generated reference pages, initializer sources, assets, "
