...
def expected_program_routes(root: Path, book: Path) -> list[str]:
    source_directory = root / "docs/reference/programs"
    problems: list[str] = []
    if not source_directory.is_dir():
        return ["docs/reference/programs: generated program reference directory is missing"]

    pages = sorted(source_directory.glob("*.md"))
    if not pages:
        return ["docs/reference/programs: no generated program reference pages found"]

    for page in pages:
        text = page.read_text(encoding="utf-8")
        if not text.startswith(GENERATED_PROGRAM_MARKER):
            problems.append(
                f"{page.relative_to(root)}: generated program page marker is missing"
            )
        route = Path("reference/programs") / page.with_suffix(".html").name
        if not (book / route).is_file():
            problems.append(f"{book}: missing generated program page: {route}")
    return problems
...
def main() -> int:
    ...
    if not book.is_dir():
        ...
        problems.extend(expected_program_routes(root, book))
